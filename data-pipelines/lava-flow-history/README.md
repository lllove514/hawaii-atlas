...
play and the flows erupt in chronological order, from the ~700,000-year-old
Kohala shields to the 2022 Mauna Loa eruption. Each younger flow buries the
older ground it covered.
...
Hawaiʻi Island is built almost entirely of overlapping lava flows. On a static
geologic map it is hard to see time: which ground is fresh and which is ancient.
This map rasterizes the flows into a single youngest-flow-wins surface and
animates the reveal, so the resurfacing plays out in order.
...
Everything fits on one screen, with no scrolling to reach the controls.
...
* **Reading it**: the caption names the era you’re watching ("The Kohala and
  Mauna Kea shields take shape…", "The historic era…"); the readout gives a monotonic frontier ("Revealed
...
```
python scripts/discover_source.py   # verify sources
python scripts/download.py          # download + reproject
python scripts/build_rasters.py     # hillshade + flow-age
python scripts/export_web.py        # browser payload
...
printing a verification report and asserting its own checks.
...
* **Recency.** DS-144 was compiled ~2000–2005, so its own dated flows end ~2003.
  ...
  2020–2024** (largely confined to the caldera floor and a few pixels at this
  scale) and any 2018–2022 Puʻuʻōʻō/summit activity without a published
...
* **The slider is ordinal, not linear in time.** Distinct ages are evenly
  ...
  maps to **log-age**, described below, so the two axes disagree on purpose.)
