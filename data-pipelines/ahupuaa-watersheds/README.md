...
see where the old boundaries and the actual drainage agree — and where they part.
...
line up with a single computed drainage basin. Often it does. Sometimes it
doesn't — because these boundaries are cultural and legal, not the output of a
flow-accumulation algorithm, and the divergence is the interesting part.
...
- **Terrain / Match views.** Terrain shows the hillshade, basins and streams.
  **Match** recolours every ahupuaʻa by how well its boundary matches a single
  ...
  plus others, red = the drainage splits across several. This turns the central
  question into the headline picture.
...
- **Search** an ahupuaʻa by name (diacritic-insensitive) to pin it, and
  **Show biggest mismatches** to highlight the handful of divisions whose
  boundaries diverge most from the computed drainage — the most interesting cases.
...
```bash
python scripts/discover_source.py   # Phase 0 — verify both sources
python scripts/download.py          # Phase 1 — download + align (resumable)
python scripts/flow.py              # Phase 2 — condition, route, streams, basins
python scripts/export_web.py        # Phase 3 — export the web payload
python scripts/verify_web.py        # Phase 4 — hover hit-test + overlay check
...
- **Coastline basins.** On a 10 m coast, thousands of single-cell strips drain
  ...
  drawn and used in the comparison. The count is reported honestly in QA.md.
...
See `QA.md` for the per-phase verification results and the final self-review.
