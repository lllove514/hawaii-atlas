...
- **What it does:** NOAA Coral Reef Watch 5 km daily SST around the islands,
  ...
  (23–28 °C stepped scale), Anomaly (diverging, vs per-pixel week-of-year mean
  computed in the browser), DHW (stepped bands, thresholds at 4/8) — plus
...
- **Verified working:** loads (with progress bar), renders all three layers,
  ...
  rendering). Pixel-art aesthetic is consistent and deliberate.
...
- **Code quality:** front end and pipeline are clean, commented for the reader,
  relative paths throughout. `README.md` and `QA.md` are strong and honest.
...
- **Docs:** per-project READMEs are genuinely good (sources, method, honest
  limitations). QA.md files record real verification evidence — worth keeping,
...
- Reef grids now ship gzipped (`sst.i16.gz` 8.7 MB, `dhw.u8.gz` 0.42 MB —
  98 MB → 9.1 MB, no loss, weekly resolution kept). `build_web.py` writes the
...
| Ahupuaʻa: more islands (Kauaʻi/Maui/Hawaiʻi) | Skip | Pipeline is island-parameterised and documented, but each island is hours of DEM download + routing; nothing to verify into the repo tonight. The island switcher honestly lists them as not yet processed. |
...
- **Lava — volcano surface shares**: Volcano-mode legend now shows each
  ...
  caption carries the story beat ("Five shield volcanoes built the island —
  Mauna Loa alone paved 50% of its surface.").
...
- Top-level `README.md`: story, per-map table, layout, tech approach, local
  run + Pages deploy, honest limitations, data credits.
